...
export const RegistrarCalidad = ({ onVolver }: { onVolver: () => void }) => {
  ...
  const procesarCalidad = () => {
    ...
    if (incompleto) return alert("❗ Error: Debe completar todos los valores.");
  ...
  return (
    <div className="flex items-center justify-center min-h-screen w-full bg-black p-4 font-mono">
      <div className="border-2 border-cyan-500 p-8 bg-gray-900 shadow-[0_0_20px_rgba(6,182,212,0.2)] w-full max-w-md">
                ...
                camionesEsperandoCalidad.map((op) => (
                  ...
                  >
                    <div className="flex flex-col">
                      <span className="text-white font-bold">{op.patente}</span>
        ...
        <button
          onClick={onVolver}
          className="w-full text-red-700 text-[10px] font-bold border-t border-gray-800 pt-4 text-center mt-6 uppercase hover:text-red-500"
        >
          &lt;&lt; Menú Principal
        </button>
      </div>
